# -*- coding: utf-8 -*-
"""
08_historico_secao.py
Pos-processador: insere a secao "Evolucao Historica" (2021-2025) nos dashboards
ja gerados pelo 05_gerar_dashboards.py.

- Le os dados historicos de ANALISE/historico/*.json (calculados uma vez; estaticos).
- Le o DADOS injetado pelo 05 em cada output/{Marca}_Dashboard.html, mapeia CO->nome
  pelas medias de 2025, detecta single/multi-municipio e injeta a secao.
- Idempotente: se a secao ja existe no arquivo, pula.

Uso:
    python 08_historico_secao.py                      # todas as marcas com dashboard
    python 08_historico_secao.py "Matriz Educação"    # so uma

Chamado automaticamente pelo 05 ao final (ver 05_gerar_dashboards.py).
"""
import io, os, re, sys, json
from collections import Counter

ANALISE_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_DIR = os.path.join(ANALISE_DIR, "output")
HIST_DIR = os.path.join(ANALISE_DIR, "historico")

BENCH = json.load(open(os.path.join(HIST_DIR, "historico_benchmark.json"), encoding="utf-8"))
UNID = json.load(open(os.path.join(HIST_DIR, "historico_unidades.json"), encoding="utf-8"))
TOP = json.load(open(os.path.join(HIST_DIR, "historico_top100.json"), encoding="utf-8"))

AR = ["CN", "CH", "LC", "MT", "RD"]
UF_NOME = {"RJ": "Rio de Janeiro", "MG": "Minas Gerais", "RS": "Rio Grande do Sul",
           "SP": "São Paulo", "ES": "Espírito Santo", "PR": "Paraná", "SC": "Santa Catarina"}

# Marcas com dashboard single-brand (Multimarcas tem estrutura propria, fora do escopo).
BRAND_LIST = ["Apogeu", "QI Bilíngue", "Matriz Educação", "Cubo Global", "Colégio Leonardo da Vinci"]


def safe_nome(marca: str) -> str:
    """Mesma normalizacao de nome de arquivo do 05_gerar_dashboards.py."""
    return marca.replace(" ", "_").replace("é", "e").replace("ô", "o")
...
def mapear_nomes(marca, html):
    ...
    m = re.search(r'const DADOS\s*=\s*(\{.*?\});\s*\n', t, re.S)
    if not m:
        return {}
    d = json.loads(m.group(1))
    dash = {}
    for nome, info in d.get("unidades", {}).items():
        a = info["areas"]
...
def main():
    alvos = sys.argv[1:] if len(sys.argv) > 1 else BRAND_LIST
    for marca in alvos:
        if marca in BRAND_LIST:
            aplicar(marca)
