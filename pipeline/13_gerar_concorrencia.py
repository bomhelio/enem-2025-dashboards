...
function renderConfronto(){
  ...
  const rows = linhas.map(([nome, get]) => {
    ...
    return "<tr><td><strong>"+nome+"</strong></td><td>"+corA(fmt(sa.media))+"</td><td>"+corB(fmt(sb.media))+"</td><td>"+deltaHtml(dd)+
  ...
  document.getElementById("tblConfronto").innerHTML =
    "<thead><tr><th></th><th>"+A.label+" ★</th><th>"+B.label+"</th><th>Δ</th><th>Mediana</th>"+
    ...
    "Nas colunas Mediana, P90, ≥600 e ≥700, cada par traz primeiro a unidade ★ e depois o concorrente, nas cores de cada um. · "+
