...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .stat span span { display:inline; margin-top:0; }
  .stat .rede-dot { display:inline-block; }
...
(function(){
  ...
  // Marca sem base 2025 (referência 2024): em vez de um card vazio por rede
  // ("sem praça em comum"), um card único lista as redes concorrentes.
  const ngMarca25 = ((r[MARCA]||{}).nota_geral||{}).media;
  REDES.forEach(rd => {
    const x = r[rd];
    if (rd === MARCA) {
      if (ngMarca25 != null) {
        cards.push({t: rd, v: fmt(ngMarca25), dot: CORES[rd],
        ...
        cards.push({t: rd, v: fmt(ng24), dot: CORES[rd],
          s: "<strong>referência ENEM 2024</strong> · sem vínculo em 2025"});
      }
      return;
    }
    if (ngMarca25 == null) return;
  ...
  if (ngMarca25 == null) {
    const outras = REDES.filter(rd => rd !== MARCA);
    cards.push({t: "Redes concorrentes", v: fmtInt(outras.length),
      s: outras.map(rd => '<span style="white-space:nowrap">'+dot(rd)+rd+'</span>').join(" · ")+
         " — pareado indisponível sem base 2025; ver a Batalha territorial"});
  }
