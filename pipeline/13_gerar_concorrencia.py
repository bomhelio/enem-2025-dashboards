...
const nomeCurto = u => u.label.replace("Matriz — ","").replace("Elite — ","").replace("Santa Mônica — ","");
...
(function(){
  ...
  DATA.unidades.filter(u=>u.nossa).forEach(u =>
    selA.insertAdjacentHTML("beforeend", '<option value="'+u.co+'">'+nomeCurto(u)+" · "+u.municipio+"</option>"));
  REDES.forEach(rd => {
    ...
    grupo.sort((a,b)=>(ngOf(b)||0)-(ngOf(a)||0)).forEach(u => {
      ...
      o.textContent = nomeCurto(u)+" · "+u.municipio+" (NG "+fmt(ngOf(u))+")";
      og.appendChild(o);
    });
    selB.appendChild(og);
  });
